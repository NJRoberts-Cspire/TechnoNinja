...
on_activate():
    slot_level = use_spell_slot(Owner, 4, 'Leomund\'s Secret Chest')
    if slot_level == -1:
        return
    name = Owner.name
    log('{{name}} hides a chest on the Ethereal Plane; can be recalled at any time using the tiny replica')
