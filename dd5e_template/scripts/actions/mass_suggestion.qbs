...
on_activate():
    slot_level = use_spell_slot(Owner, 6, 'Mass Suggestion')
    if slot_level == -1:
        return
    if !Scene:
        failed = resolve_spell_save_manually(Owner, 'Wisdom')
        if !failed:
            log('Target fails Mass Suggestion Wisdom save')
        else:
            log('Target succeeds Mass Suggestion Wisdom save')
        return
    ...
    for targ in successes:
        name = targ.name
        log('{{name}} resisted Mass Suggestion')
