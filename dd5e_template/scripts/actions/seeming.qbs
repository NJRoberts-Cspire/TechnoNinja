...
on_activate():
    slot_level = use_spell_slot(Owner, 5, 'Seeming')
    if slot_level == -1:
        return
    if !Scene:
        prompt('Seeming: select targets in an active scene to resolve this spell.', ['Ok'])
        return

    targets = selectCharacters()
    name = Owner.name
    log('{{name}} changes the appearances of selected creatures for 8 hours')
