...
on_activate():
    slot_level = use_spell_slot(Owner, 4, 'Mordenkainen\'s Private Sanctum')
    if slot_level == -1:
        return
    name = Owner.name
    log('{{name}} creates a magical ward on an area of up to 100-foot cube; blocks scrying, teleportation, and planar travel; sound cannot pass through')
