...
on_activate():
    slot_level = use_spell_slot(Owner, 3, 'Leomund\'s Tiny Hut')
    if slot_level == -1:
        return
    name = Owner.name
    log('{{name}} creates a 10-foot-radius dome of force around their position; up to 9 creatures inside are protected from the elements for 8 hours')
