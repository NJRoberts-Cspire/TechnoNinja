...
**Components**: V, S, M (a pinch of salt and one copper piece placed on each of the corpse’s eyes, which must remain there for the duration) 148
...
on_activate():
    slot_level = use_spell_slot(Owner, 2, 'Gentle Repose')
    if slot_level == -1:
        return
    name = Owner.name
    log('{{name}} casts Gentle Repose')
