...
on_activate():
    ...
    for targ in failures:
        name = targ.name
        log('{{name}} is polymorphed; transformation becomes permanent if concentration is maintained for 1 hour')
    for targ in successes:
        name = targ.name
        log('{{name}} resisted True Polymorph')
