...
on_activate():
    slot_level = use_spell_slot(Owner, 4, 'Fabricate')
    if slot_level == -1:
        return
    name = Owner.name
    log('{{name}} converts raw materials into a finished item; the materials must be in sight and the product must fit within a 10-foot cube')
