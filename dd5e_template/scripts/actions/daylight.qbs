...
on_activate():
    slot_level = use_spell_slot(Owner, 3, 'Daylight')
    if slot_level == -1:
        return
    name = Owner.name
    log('{{name}} causes a chosen point to shed bright light in a 60-foot radius and dim light for an additional 60 feet for 1 hour')
