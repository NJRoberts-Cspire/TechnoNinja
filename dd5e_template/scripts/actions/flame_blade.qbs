...
on_activate():
    slot_level = use_spell_slot(Owner, 2, 'Flame Blade')
    if slot_level == -1:
        return
    base_dice = 3 + floor((slot_level - 2) / 2)
    set_concentration(Owner, 'Flame Blade')
    name = Owner.name
    log('{{name}} conjures a Flame Blade. On a hit: {{base_dice}}d6 fire damage')
