...
on_activate():
    slot_level = use_spell_slot(Owner, 2, 'Spike Growth')
    if slot_level == -1:
        return
    set_concentration(Owner, 'Spike Growth')
    caster_name = Owner.name
    log('{{caster_name}} casts Spike Growth: creatures moving through the area take 2d4 piercing damage per 5 feet traveled')
