...
on_activate():
    slot_level = use_spell_slot(Owner, 9, 'Storm of Vengeance')
    if slot_level == -1:
        return
    set_concentration(Owner, 'Storm of Vengeance')
    if !Scene:
        failed = resolve_spell_save_manually(Owner, 'Constitution')
        if !failed:
            log('Target fails Storm of Vengeance Constitution save')
        else:
            log('Target succeeds Storm of Vengeance Constitution save')
        return
    ...
    for targ in failures:
        damage = roll_damage(Owner, '3d6', targ, 'Thunder')
        set_condition(targ, 'Deafened', true)
        name = targ.name
        log('Storm of Vengeance hits {{name}} for {{damage}} thunder damage and Deafens them; storm rages with additional effects each round')
        apply_damage(targ, damage)
    for targ in successes:
        damage = roll_damage(Owner, '3d6', targ, 'Thunder')
        half_damage = floor(damage / 2)
        name = targ.name
        log('Storm of Vengeance grazes {{name}} for {{half_damage}} thunder damage')
