...
**Components**: V, S, M (a forked, metal rod worth at least 250 gp, attuned to a particular plane of existence) 168
...
on_activate():
    slot_level = use_spell_slot(Owner, 7, 'Plane Shift')
    if slot_level == -1:
        return
    if !Scene:
        prompt('Plane Shift: select targets in an active scene to resolve this spell.', ['Ok'])
        return

    targ = selectCharacter()
    name = targ.name
    log('{{name}} is shifted to another plane')
