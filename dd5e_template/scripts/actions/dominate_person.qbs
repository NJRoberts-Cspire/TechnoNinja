...
on_activate():
    slot_level = use_spell_slot(Owner, 5, 'Dominate Person')
    if slot_level == -1:
        return
        set_concentration(Owner, 'Dominate Person')
    if !Scene:
        failed = resolve_spell_save_manually(Owner, 'Wisdom')
        if !failed:
            log('Target fails Dominate Person Wisdom save')
        else:
            log('Target succeeds Dominate Person Wisdom save')
        return
    ...
    for targ in successes:
        name = targ.name
        log('{{name}} resisted Dominate Person')
