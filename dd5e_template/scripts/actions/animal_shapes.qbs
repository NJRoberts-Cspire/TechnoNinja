...
on_activate():
    slot_level = use_spell_slot(Owner, 8, 'Animal Shapes')
    if slot_level == -1:
        return

    name = Owner.name    
    set_concentration(Owner, 'Animal Shapes')
    prompt('You transform any number of willing creatures into large or smaller creatures.', ['Ok'])
    log('{{name}} transforms selected creatures into beasts')
