...
on_activate():
    slot_level = use_spell_slot(Owner, 3, 'Conjure Animals')
    if slot_level == -1:
        return
    set_concentration(Owner, 'Conjure Animals')
    name = Owner.name
    log('{{name}} conjures fey spirits that take the form of beasts; they appear in unoccupied spaces and act as allies')
