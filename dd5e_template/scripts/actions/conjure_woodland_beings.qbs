...
on_activate():
    slot_level = use_spell_slot(Owner, 4, 'Conjure Woodland Beings')
    if slot_level == -1:
        return
    set_concentration(Owner, 'Conjure Woodland Beings')
    name = Owner.name
    log('{{name}} summons fey creatures of challenge rating 2 or lower; higher slot levels allow more powerful or numerous fey')
