...
on_activate():
    slot_level = use_spell_slot(Owner, 4, 'Giant Insect')
    if slot_level == -1:
        return
    set_concentration(Owner, 'Giant Insect')
    name = Owner.name
    log('{{name}} transforms up to 10 centipedes, 3 spiders, 5 wasps, or 1 scorpion into giant versions that act as allies')
