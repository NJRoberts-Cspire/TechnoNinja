...
on_activate():
    slot_level = use_spell_slot(Owner, 4, 'Divination')
    if slot_level == -1:
        return
    name = Owner.name
    log('{{name}} contacts a divine power for a truthful reply about a specific goal, event, or activity occurring within 7 days')
