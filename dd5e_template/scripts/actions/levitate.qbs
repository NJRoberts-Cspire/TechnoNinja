...
on_activate():
    slot_level = use_spell_slot(Owner, 2, 'Levitate')
    if slot_level == -1:
        return
    set_concentration(Owner, 'Levitate')
    if !Scene:
        prompt('Levitate: select targets in an active scene to resolve this spell.', ['Ok'])
        return

    targ = selectCharacter()
    name = targ.name
    log('{{name}} is levitated up to 20 feet vertically by Levitate')
