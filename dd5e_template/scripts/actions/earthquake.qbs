...
on_activate():
    slot_level = use_spell_slot(Owner, 8, 'Earthquake')
    if slot_level == -1:
        return
    set_concentration(Owner, 'Earthquake')
    if !Scene:
        failed = resolve_spell_save_manually(Owner, 'Constitution')
        if !failed:
            log('Target fails Earthquake Constitution save')
        else:
            log('Target succeeds Earthquake Constitution save')
        return
    ...
    for targ in failures:
        ...
        name = targ.name
        log('{{name}} is knocked Prone; ground is difficult terrain and concentration checks are required')
    for targ in successes:
        name = targ.name
        log('{{name}} maintains footing during the Earthquake')
