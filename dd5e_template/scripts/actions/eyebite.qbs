...
on_activate():
    slot_level = use_spell_slot(Owner, 6, 'Eyebite')
    if slot_level == -1:
        return
    set_concentration(Owner, 'Eyebite')
    name = Owner.name
    log('{{name}} casts Eyebite; each turn one creature can be targeted with Asleep, Panicked, or Sickened')
